use crate::{
    journal_sys::{
        sd_id128_get_boot, sd_id128_get_machine, sd_id128_t as SdId128, sd_id128_to_string,
    },
    SdResult,
};
use std::{fmt, mem, os::raw::c_char};
...
impl fmt::Display for Id128 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut buf = [0u8; 33];
        unsafe {
            sd_id128_to_string(self.0, &mut buf as *mut _ as *mut c_char);
        }
...
impl fmt::Debug for Id128 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("Id128").field(&self.to_string()).finish()
    }
}
